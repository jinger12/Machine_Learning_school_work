...
import sys
import utils.data_cleaning_utils as data_cleaning_utils
...
def out_of_pipeline_preproc(df, target_attr, missingness_threshold=0.20, model_dev_drop_list=None):

    if model_dev_drop_list is None:
        model_dev_drop_list = []

    print('\n', 80 * '*', sep='')
    print('out of pipeline preprocessing')

    # drop observations with nan in target attribute
    print(f'\nnan count in target attribute before out of pipeline preprocessing: {df[target_attr].isna().sum()}')
    df = df.dropna(subset=target_attr)
    print(f'nan count in target attribute after out of pipeline preprocessing: {df[target_attr].isna().sum()}')

    # drop attributes with nan count > missingness_threshold
    cap_x_df = df.drop(columns=target_attr)
    cap_x_df, drop_list = \
        data_cleaning_utils.drop_attrs_gt_missingness_threshold(cap_x_df, missingness_threshold=missingness_threshold)
    df = pd.concat([cap_x_df, df[target_attr]], axis=1)
    print(f'\nattributes dropped for exceeding missingness_threshold = {missingness_threshold}: ', drop_list)

    # attribute drops based on model development
    print(f'\nmodel_dev_drop_list: {model_dev_drop_list}')
    df = df.drop(columns=model_dev_drop_list)
...
def get_class_preproc_pipeline(numerical_attr, nominal_attr, class_eval_dict):

    binary = class_eval_dict['binary']
    if binary:
        preprocessor = build_preprocessing_pipeline(numerical_attr, nominal_attr, 'binary')
    else:
        preproc_dict = mc_te_utils.get_cat_enc_mul_cla_tar_enc_preproc_pipeline(numerical_attr, nominal_attr)
...
def fit_collection_of_estimators_classification(numerical_attr, nominal_attr, estimator_names, estimator_list, cap_x_df,
                                                y_df, data_set_type, model_selection_stage, class_eval_dict):
